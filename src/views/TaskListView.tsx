...
const TaskListView: React.FC<TaskListViewProps> = ({ 
  ...
  const toggleExpand = (taskId: string) => {
    ...
    // Clicking a checked/collapsed task must not trigger the subtask expansion.
    ...
    >
      <div className={`task-item themed-border ${task.completed ? 'completed' : ''} ${mergeTargetId === task.id ? 'merge-target' : ''}`} style={{ borderLeft: `4px solid ${getPriorityColor(task.priority)}`, position: 'relative' }}>
        <div className="task-left-controls">
          ...
          <button className="chevron-trigger" onClick={() => toggleExpand(task.id)} title="Sub-tasks" style={{ opacity: task.completed ? 0 : undefined, pointerEvents: task.completed ? 'none' : 'auto' }}>
